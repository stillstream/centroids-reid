...
if __name__ == "__main__":
    ...
    print("111", embeddings.shape)
    print("222", embeddings_gallery.shape)
    ...
    print("DISTANCE_FUNC", cfg.SOLVER.DISTANCE_FUNC)
    dist_func = get_dist_func(cfg.SOLVER.DISTANCE_FUNC)
    # print("777", embeddings.shape, type(embeddings), embeddings.dtype)
    # print("777", embeddings_gallery.shape, type(embeddings_gallery), embeddings_gallery.dtype)
    ...
    with open(SAVE_DIR / "results.txt", "w") as f:
        for key, value in out.items():
            line = str(key) + " " + str(value["distances"]) + "\n"
            f.write(line)
